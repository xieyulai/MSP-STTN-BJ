...
class Dataset:
    ...
    def trainset_of(self, vec):
        return vec[:math.floor((len(vec) - self.len_test + 20) * self.portion)]

    def testset_of(self, vec):
        return vec[-math.floor(self.len_test * self.portion) + 20:]
    ...
    def load_data(self):
        ...
        print('# timestamp',ts_y[20:-28][-self.len_test+20:])

        Y_Class = []
        for i in range(len(ts_new_list)):
            Y_Class.append(np.array(range(20, 48)))
            for j in range(len(ts_y_list[i])//48-1):
        ...
        x_con_tes = x_con_tes[:-28]
        x_ave_tes = x_ave_tes[:-28]
        x_ave_q_tes = x_ave_q_tes[:-28]
        y_tes = y_tes[:-28]
        y_cls_tes = y_cls_tes[:-28]
        y_typ_tes = y_typ_tes[:-28]
...
if __name__ == '__main__':
    ...
    df = DatasetFactory(DataConfiguration(0, 1, 1), Inp_type='external', Data_type='Sub', Length=6, Is_seq=0)
