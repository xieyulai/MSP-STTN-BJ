...
def run(mcof):
    ...
    if IS_TRAIN:
        ...
        if Keep_Train:
            path = './model/All/MinMax/Short/Imp_{}/pre_model_{}.pth'.format(PRESUME_RECORD_ID, PRESUME_EPOCH_S)
    ...
    if IS_VAL:
        ...
        if 1:
            ...
            for epoch in range(EVAL_START_EPOCH, EPOCH_E):
                ...
                info = "=%s= %s:%d/%d ep:%d MAE: %.4f RMSE: [%.4f] IN: %.4f OUT: %.4f TIM: %.4f Dur:%s" % (RECORD_ID,NAME,(epoch+1)*MUL,TOT,C_EP,mae,rmse,rmse_in, rmse_out,tim_acc,test_epoch_time)###xie
                #print(info)  ###xie
